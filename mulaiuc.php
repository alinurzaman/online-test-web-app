...
<head>
	...
	<title>UC ONLINE LBB BEST | MULAI UC</title>
						...
						<div class="col-md-12">
							<div class="panel panel-default card-view">
								<div class="panel-heading">
									<div class="pull-left">
										<h6 class="panel-title txt-dark"><?php echo $data_uc['nama_uc'];?></h6>
									</div>
									<div class="clearfix"></div>
								</div>
								<form method="post" action="">
									<div class="col-md-2">
										<input type="password" class="form-control" name="pin" placeholder="PIN UC" maxlength="6">
									</div>
									<div class="col-md-4">
										<button name="submit" type="submit" class="btn btn-success  mr-10">Submit</button>
									</div>
								</form>
								<div class="clearfix"></div>
								<br>
							<?php if(isset($_POST['submit'])==true){
												$pin = $_POST['pin'];
												if ($pin == $data_uc['pin_uc']){
													$get_materi = mysql_query("SELECT m.id_materi AS id, m.tipe_materi AS tipe, m.ujian_materi AS ujian, m.batasnilai_materi AS batas, m.waktu_materi AS waktu, h.nilai_hasil AS nilai FROM (SELECT * FROM materi WHERE id_uc='$id_uc') m LEFT JOIN (SELECT * FROM hasil WHERE id_siswa = '$id_siswa') h ON m.id_materi = h.id_materi");
													
							?>
							<br>
							<strong>
							Perhatian : <br>
							1. Setelah klik tombol 'Mulai', anda akan langsung melaksanakan ujian yang dipilih.<br>
							2. Nilai yang disimpan di database adalah nilai dari ujian pertama. Nilai ujian ulang tidak akan disimpan.<br>
							3. Mohon pastikan koneksi internet lancar.<br>
							</strong>
									...
									<div class="table-wrap">
										<div class="table-responsive">
											<table id="datable_1" class="table table-hover table-bordered display mb-30" >
												...
												<tbody>
													<?php $s="T"; $i=1; while($data = mysql_fetch_array($get_materi)){?>
													<tr>
														<td><?php echo $i; ?></td>
														<td><?php echo $data['tipe']; ?></td>
														<td><?php echo $data['ujian']; ?></td>
														...
														?>
														<td><a href="preujian.php?idm=<?php echo $data['id'];?>&idu=<?php echo $enc; ?>&ret=<?php echo $ret; ?>"><button class="btn btn-primary btn-anim"><i class="fa fa-pencil-square-o"></i><span class="btn-text">Mulai</span></button></a></td>
														<?php } else { ?> <td>-</td> <?php } ?>
														<?php 
															if($data['nilai']==NULL){
																$s = "Y";
													...
													?>
													<tr>
														<td colspan="5" style="text-align: right; font-weight: bold">HASIL KESELURUHAN</td>
														<td style="text-align: left">
															<?php if($s=="T"){?>
															<a href="rekaphasil.php?idu=<?php echo $id_uc;?>"><button class="btn btn-info"><span class="btn-text">Lihat</span></button></a>
															<?php } else{?>
